...
class user_funcs {
	public static function create_user($s_username, $s_password, $s_email, $a_other = NULL, &$s_feedback = "") {
		...
		$a_users = db_query("SELECT `id` FROM `[maindb]`.`users` WHERE `username`='[username]'",
		...
		// create the user
		db_query("INSERT INTO `[maindb]`.`users` (`username`,`pass`,`email`,`accesses`,`creation`,`campaigns`,`characters`) VALUES ('[username]',AES_ENCRYPT('[username]','[password]'),'[email]','[accesses]','[creation]','[campaigns]','[characters]')",
				 array('maindb'=>$maindb, 'username'=>$s_username, 'password'=>$s_password, 'email'=>$s_email, 'accesses'=>$s_access, 'creation'=>date("Y-m-d H:i:s"), 'campaigns'=>'', 'characters'=>''));
	...
	public static function email_notification($s_username, $s_header, $s_body) {
		...
		$a_user = db_query("SELECT `email` FROM `users` WHERE `username`='[username]'", array('username'=>$s_username));
		if (count($a_user) == 0)
				return FALSE;
	...
	public static function reset_password($s_username, $s_key, $s_password, $b_force = FALSE) {
		...
		// reset the password
		db_query("UPDATE `[maindb]`.`users` SET `pass`=AES_ENCRYPT('[username]','[password]') WHERE `username`='[username]'", array("username"=>$s_username, "password"=>$s_password, "maindb"=>$maindb));
	...
	public static function username_status($s_username) {
		...
		if (strlen($s_username) == 0)
				return 'blank';
		$a_usernames = db_query("SELECT `id` FROM `[maindb]`.`users` WHERE `username`='[username]'",
								array('maindb'=>$maindb, 'username'=>$s_username));
