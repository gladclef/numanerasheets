...
function initializeUserData() {
	...
	db_query("INSERT INTO `[maindb]`.`users` (`username`,`pass`,`accesses`,`email`) VALUES ('guest',AES_ENCRYPT('guest','guest'),'','')",
		array("maindb"=>$maindb));
	$a_guest_id = db_query("SELECT `id` FROM `[maindb]`.`users` WHERE `username`='guest'",
		array("maindb"=>$maindb));
