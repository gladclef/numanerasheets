...
class ajax {
	...
	/**
	 * Get all of the users in the `users` table as a json string
	...
	function get_full_users_list($b_ignore_disabled = TRUE) {
		...
		$a_query_results = db_query("SELECT `username`,`email`,`disabled` FROM `[maindb]`.`users` {$s_disabled}", array("maindb"=>$maindb));
